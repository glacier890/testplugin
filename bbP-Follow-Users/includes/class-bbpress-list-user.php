...
class BBPressList_User {
  ...
  */
  ...
  function get_followers( $user_id ) {
    ...
    return apply_filters( 'bbp_follow_get_followers', $followers, $user_id );
  ...
  */
  ...
  $following = get_user_meta( $user_id, '_bbpresslist_following', true);
    ...
    return apply_filters('bbp_follow_get_following', $following, $user_id );
  ...
  */
  ...
  function add_user_to_list( $user_id, $follower_id ) {
   ...
   do_action( 'bbp_list_user_added_following', $user_id, $users_in_list );
   update_user_meta ( $follower_id, '_bbpresslist_followers', $followers );
   do_action( 'bbp_list_user_added_followers', $user_id, $followers );
   return 'follow';
    ...
    // get all IDs that follow the user we have just unfollowed so that we can remove $user_id
      ...
      foreach ( $followers as $key => $follower ) {
        ...
        if ( $follower == $user_id ) {
         ...
         do_action( 'bbp_list_user_unfollow', $unfollow_user, $followers );
  		}
  	}
  	return 'unfollow';
  }
